...
const CartPage = () => {
  ...
  if (!product) {
    return <div className="text-center text-white mt-32">Product not found</div>;
  }
    ...
    <div className="relative min-h-screen pt-32 pb-16 px-4 md:px-8 flex justify-center items-center">
      <div className="bg-gray-800 text-textColor2 p-8 md:p-12 rounded-lg shadow-lg max-w-4xl w-full flex flex-col md:flex-row">
        <div className="md:w-1/2 flex justify-center mb-8 md:mb-0">
          <img src={product.image} alt={product.name} className="w-full h-auto max-h-[500px] object-cover rounded-lg" />
        </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex space-x-2 mt-2">
                {product.colors.map((color, index) => (
                  <div key={index} className="w-8 h-8 rounded-full border border-gray-300" style={{ backgroundColor: color }}></div>
                ))}
              </div>
            </div>
            <div className="mb-4">
              ...
              <div className="flex space-x-2 mt-2">
                {product.sizes.map((size, index) => (
                  <button key={index} className="px-4 py-2 border rounded hover:bg-gray-200 transition">{size}</button>
                ))}
              </div>
            </div>
            <div className="mb-4">
